...
int c_socket_server::connect(int port)
{
	...
	if ((rc = getaddrinfo(nullptr, std::to_string(port).c_str(), &hints, &add_info)) != 0)
	{
		DLOG(ERROR) << "Failed to get address information. "
			<< gai_strerror(rc);
		return -2;
	}
...
int c_socket_server::wait_for_incoming_connection(void)
{
	...
	memset((char *)&client_addr,0,sizeof(client_addr));        /*clear sockaddr structure*/
	...
	DLOG(INFO) << "Server: waiting for connections on descriptor = " << descriptor;
	...
	while (1)
	{
		...
		buff = "This server has been contacted " + std::to_string(++visits)
			+ " time";
...
int c_socket_client::connect(const std::string& host, int port)
{
	...
	if ((rc = getaddrinfo(host.c_str(), std::to_string(port).c_str(),
					&hints, &add_info)) != 0)
	{
		DLOG(ERROR) << "Failed to get address information. "
			<< gai_strerror(rc);
	...
	{
		if ((s_desc = socket(ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol)) < 0)
		{
